...
class TestPatternConstructors:
    ...
    def test_helical_size_scales_pattern(self):
        ...
        p2 = ps.helical(size=2)
        p3 = ps.helical(size=3)
        # Larger size → more lines
    ...
    def test_enterprise_n2(self):
        ...
        p = ps.enterprise(N=2)
        assert len(p.pattern) > len(ps.enterprise(N=1).pattern)
...
class TestFlowDiverterMesh:
    ...
    def test_node_count_matches_layers(self, simple_helical_stent):
        ...
        # After clean() some isolated points may be removed; actual <= expected
    ...
    def test_offset_angle_shifts_pattern(self):
        """Two stents with different offset_angle must have different point coordinates."""
        import PyStenting as ps

        pattern = ps.helical(size=2)
        fd0 = ps.FlowDiverter(pattern, radius=1.5, height=10, tcopy=6, hcopy=3, offset_angle=0.0)
        fd1 = ps.FlowDiverter(pattern, radius=1.5, height=10, tcopy=6, hcopy=3, offset_angle=0.25)
        # At least some points must differ
